...
-- Dumping data for table chaira_sigepi.pe_archivo: ~3 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_area_trabajo: ~4 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_dependecia_proyecto: ~3 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_evaluacion: ~4 rows (approximately)
...
REPLACE INTO `pe_evaluacion` (`ID_EVALUACION`, `EVA_OBSERVACIONES`, `ID_PE_PROYECTO`, `EVA_PUNTAJE_TOTAL`, `ID_PE_PERSONA`, `EVA_FECHA`, `EVA_ESTADO`) VALUES
	(1, 'yISUS', 1, 60, 11, '2020-03-02 14:25:48', '1'),
	(2, 'Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo', 2, 0, 12, '2020-04-23 16:44:36', '0'),
	(3, 'But I must explain to you how all this mistaken idea of denouncing pleasure and praising pain was born and I will give you a complete account of the system, and expound the actual teachings of the great explorer of the truth, the master-builder of human happiness. ', 3, 0, 11, '2020-03-25 16:45:06', '0'),
	(4, 'algo', 3, 0, 13, '2020-03-02 00:52:29', '0');
...
-- Dumping data for table chaira_sigepi.pe_evaluacion_item: ~28 rows (approximately)
...
REPLACE INTO `pe_evaluacion_item` (`ID_EVALUACION_ITEM`, `EVIT_PUNTAJE`, `ID_PE_EVALUACION`, `ID_PE_ITEM`, `EVIT_FECHA_CALIFICACION`) VALUES
	(1, 10, 1, 1, '2020-03-02 14:25:48'),
	(2, 5, 2, 2, '2020-02-28 16:46:47'),
	(3, 2, 3, 3, '2020-02-28 16:47:10'),
	(4, 3, 3, 5, '2020-02-28 16:49:32'),
	(5, 10, 1, 2, '2020-03-02 14:25:48'),
	(6, 10, 1, 3, '2020-03-02 14:25:48'),
	(7, 10, 1, 4, '2020-03-02 14:25:48'),
	(8, 10, 1, 5, '2020-03-02 14:25:48'),
	(9, 10, 1, 6, '2020-03-02 14:25:48'),
	(10, 0, 1, 7, '2020-03-01 23:54:24'),
...
-- Dumping data for table chaira_sigepi.pe_facultad: ~6 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_grupo_extension: ~7 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_item: ~8 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_objetivos: ~3 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_persona: ~13 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_persona_programa: ~3 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_poblacion: ~4 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_poblacion_ciclo: ~5 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_programa: ~6 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_proyecto: ~3 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_proyecto_persona: ~7 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_proyecto_poco: ~2 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_proyecto_pogr: ~3 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_rol: ~8 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_tipo_dependencia: ~2 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_tipo_vinculacion: ~5 rows (approximately)
...
-- Dumping data for table chaira_sigepi.pe_usuario: ~13 rows (approximately)
...
-- Dumping structure for procedure chaira_sigepi. consul_item
DROP PROCEDURE IF EXISTS ` consul_item`;
DELIMITER //
CREATE PROCEDURE ` consul_item`(
	IN `EVAL` INT
)
BEGIN

SELECT eva_item.EVIT_PUNTAJE PUNTAJE,pe_item.ITE_NOMBRE ITEM_NOMBRE, eva_item.EVIT_FECHA_CALIFICACION FECHA FROM pe_item
INNER JOIN pe_evaluacion_item eva_item ON eva_item.ID_PE_ITEM=pe_item.ID_ITEM 
WHERE eva_item.ID_PE_EVALUACION=EVAL AND pe_item.ID_ITEM<>5;

END//
DELIMITER ;

-- Dumping structure for procedure chaira_sigepi.asignar_Puntaje_Item
DROP PROCEDURE IF EXISTS `asignar_Puntaje_Item`;
DELIMITER //
CREATE PROCEDURE `asignar_Puntaje_Item`(
...
-- Dumping structure for procedure chaira_sigepi.consultar_Observaciones_Evaluacion
DROP PROCEDURE IF EXISTS `consultar_Observaciones_Evaluacion`;
DELIMITER //
CREATE PROCEDURE `consultar_Observaciones_Evaluacion`(
	IN `ID_EVAL` INT
)
BEGIN
...
-- Dumping structure for procedure chaira_sigepi.Consul_Es_par
DROP PROCEDURE IF EXISTS `Consul_Es_par`;
DELIMITER //
CREATE PROCEDURE `Consul_Es_par`(
	IN `ID_PER` INT
)
BEGIN
Declare exite int;
...
-- Dumping structure for procedure chaira_sigepi.consul_Evaluaciones
DROP PROCEDURE IF EXISTS `consul_Evaluaciones`;
DELIMITER //
CREATE PROCEDURE `consul_Evaluaciones`()
BEGIN

SELECT pro.ID_PROYECTO ID,
eva.ID_EVALUACION EVALUACION,
pro.PRO_NOMBRE PROYECTO ,
CONCAT (per.PER_NOMBRES ," ", per.PER_APELLIDOS) AS EVALUADOR,
eva.EVA_PUNTAJE_TOTAL PUNTAJE,
eva.EVA_ESTADO ESTADO,
eva.EVA_FECHA FECHA


 from pe_persona per
INNER JOIN pe_evaluacion eva ON per.ID_PERSONA =eva.ID_PE_PERSONA
INNER JOIN pe_proyecto pro ON pro.ID_PROYECTO=eva.ID_PE_PROYECTO

ORDER BY eva.EVA_FECHA DESC
;
END//
DELIMITER ;

-- Dumping structure for procedure chaira_sigepi.consul_Evaluaciones_Detalle
DROP PROCEDURE IF EXISTS `consul_Evaluaciones_Detalle`;
DELIMITER //
CREATE PROCEDURE `consul_Evaluaciones_Detalle`(
	IN `EVALUACION` INT
)
BEGIN
...
eva.EVA_FECHA FECHA,
arc.ARC_URL ARCHIVO,
eva.EVA_OBSERVACIONES OBSERVACIONES 
...
WHERE EVALUACION=eva.ID_EVALUACION
;
...
-- Dumping structure for procedure chaira_sigepi.consul_Par_Evaluador_Docentes
DROP PROCEDURE IF EXISTS `consul_Par_Evaluador_Docentes`;
DELIMITER //
CREATE PROCEDURE `consul_Par_Evaluador_Docentes`()
BEGIN
...
-- Dumping structure for procedure chaira_sigepi.consul_Par_Evaluador_Proye
DROP PROCEDURE IF EXISTS `consul_Par_Evaluador_Proye`;
DELIMITER //
CREATE PROCEDURE `consul_Par_Evaluador_Proye`(
	IN `id_proyec` INT
)
BEGIN
...
-- Dumping structure for procedure chaira_sigepi.consul_Par_Evaluador_Proye1
DROP PROCEDURE IF EXISTS `consul_Par_Evaluador_Proye1`;
DELIMITER //
CREATE PROCEDURE `consul_Par_Evaluador_Proye1`(
	IN `id_proyec` INT
)
BEGIN
...
-- Dumping structure for procedure chaira_sigepi.consul_proyectos
DROP PROCEDURE IF EXISTS `consul_proyectos`;
DELIMITER //
CREATE PROCEDURE `consul_proyectos`()
BEGIN

SELECT pro.ID_PROYECTO ID, 
pro.PRO_NOMBRE  PROYECTO, 
CONCAT (per.PER_NOMBRES ," ", per.PER_APELLIDOS) AS COORDINADOR ,
 arc.ARC_URL ARCHIVO,
 pro.PRO_FECHA_PRESENTACION PRESENTACION,
 us.USU_CORREO CORREO
...
INNER JOIN pe_usuario us ON us.ID_PE_PERSONA=per.ID_PERSONA
...
-- Dumping structure for procedure chaira_sigepi.consul_Proyectos_del_Par
DROP PROCEDURE IF EXISTS `consul_Proyectos_del_Par`;
DELIMITER //
CREATE PROCEDURE `consul_Proyectos_del_Par`(
	IN `ID_PER` INT
...
-- Dumping structure for procedure chaira_sigepi.consul_Proyec_Evaulacion
DROP PROCEDURE IF EXISTS `consul_Proyec_Evaulacion`;
DELIMITER //
CREATE PROCEDURE `consul_Proyec_Evaulacion`(
	IN `PROYECTO` INT,
	IN `ID_USER` INT
...
-- Dumping structure for procedure chaira_sigepi.consul_proyec_prueba
DROP PROCEDURE IF EXISTS `consul_proyec_prueba`;
DELIMITER //
CREATE PROCEDURE `consul_proyec_prueba`()
BEGIN
...
-- Dumping structure for procedure chaira_sigepi.consul_Puntaje_Item
DROP PROCEDURE IF EXISTS `consul_Puntaje_Item`;
DELIMITER //
CREATE PROCEDURE `consul_Puntaje_Item`(
	IN `EVALUACION` INT
)
BEGIN
SELECT eva_item.EVIT_PUNTAJE PUNTAJE,pe_item.ITE_NOMBRE ITEM_NOMBRE, eva_item.EVIT_FECHA_CALIFICACION FECHA FROM pe_item
INNER JOIN pe_evaluacion_item eva_item ON eva_item.ID_PE_ITEM=pe_item.ID_ITEM 
WHERE eva_item.ID_PE_EVALUACION=EVALUACION AND pe_item.ID_ITEM<>5;
END//
DELIMITER ;

-- Dumping structure for procedure chaira_sigepi.consul_Puntaje_Items_Evaluador
DROP PROCEDURE IF EXISTS `consul_Puntaje_Items_Evaluador`;
DELIMITER //
CREATE PROCEDURE `consul_Puntaje_Items_Evaluador`(
	IN `id_Evaluacion` INT
)
BEGIN
...
-- Dumping structure for procedure chaira_sigepi.insert_par_evaluador
DROP PROCEDURE IF EXISTS `insert_par_evaluador`;
DELIMITER //
CREATE PROCEDURE `insert_par_evaluador`(
	IN `id_proyec` INT,
	IN `id_par` INT
...
-- Dumping structure for procedure chaira_sigepi.login
...
-- Dumping structure for procedure chaira_sigepi.prueba_cantidad
DROP PROCEDURE IF EXISTS `prueba_cantidad`;
DELIMITER //
CREATE PROCEDURE `prueba_cantidad`()
BEGIN
SELECT COUNT(proye_per.ID_PROYECTO_PERSONA ) AS cant FROM pe_rol rol
 INNER JOIN pe_usuario uss ON uss.ID_PE_ROL=rol.ID_ROL
 INNER JOIN pe_persona pers ON uss.ID_PE_PERSONA=pers.ID_PERSONA
...
-- Dumping structure for procedure chaira_sigepi.UPDA_EVALUACION
DROP PROCEDURE IF EXISTS `UPDA_EVALUACION`;
DELIMITER //
CREATE PROCEDURE `UPDA_EVALUACION`(
	IN `EVALUACION` INT,
...
BEGIN
	...
	WHERE pe_evaluacion_item.ID_PE_ITEM=6 and pe_evaluacion_item.ID_PE_EVALUACION=EVALUACION;
	 ...
	 pe_evaluacion.EVA_PUNTAJE_TOTAL=(PLANTEAMIENTO+ JUSTIFICACION+ OBJETIVOS+ METODOLOGIA+ IMPACTO + RESULTADOS)
